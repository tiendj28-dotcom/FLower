...
describe('AuthService - Login', () => {
  ...
  describe('register', () => {
    ...
    it('AuthService - REGISTER - TC-1: should register successfully with valid input', async () => {
      ...
      // INPUT
      console.log('\n📝 INPUT:', JSON.stringify(registerInput, null, 2));
      ...
      expect(UserRepository.findByPhone).toHaveBeenCalledWith(registerInput.phone);
      ...
      expect(generateRefreshToken).toHaveBeenCalledWith({ id: createdUser.id });
      expect(result).toEqual(expectedOutput);
    });
    ...
    it('AuthService - REGISTER - TC-2: should throw error when email already exists', async () => {
      ...
      // Arrange
      UserRepository.findByEmail.mockResolvedValue({ id: 1, email: registerInput.email });
      ...
      // Act & Assert
      await expect(AuthService.register(registerInput)).rejects.toThrow('Email đã được sử dụng');
    ...
    it('AuthService - REGISTER - TC-3: should throw error when phone already exists', async () => {
      ...
      // Arrange
      UserRepository.findByEmail.mockResolvedValue(null);
      UserRepository.findByPhone.mockResolvedValue({ id: 1, phone: registerInput.phone });
      ...
      // Act & Assert
      await expect(AuthService.register(registerInput)).rejects.toThrow('Số điện thoại đã được sử dụng');
    ...
    it('AuthService - REGISTER - TC-4: should throw error when username already exists', async () => {
      ...
      UserRepository.findByEmail.mockResolvedValue(null);
      UserRepository.findByPhone.mockResolvedValue(null);
      UserRepository.findByUsername.mockResolvedValue({ id: 1, username: registerInput.username });
      ...
      // Act & Assert
      await expect(AuthService.register(registerInput)).rejects.toThrow('Username đã được sử dụng');
